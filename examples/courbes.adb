...
procedure Courbes is
   ...
   Menu : constant array (0 .. 4) of String (1 .. 6) :=
     (Format_DRAW, Format_VAL, Format_ZOOM, Format_HPGL, Format_QUIT);
   ...
   begin
      ...
      loop
         ...
         loop
            ...
            Readln (Ch);
            Ch := UpCase (Ch);
            exit when (Ch = Format_YES (1)) or (Ch = Format_NO (1));
         end loop;
         Axes := Ch = Format_YES (1);
         if Axes then
            loop
               Write (Format_XSCA);
               Readln (EchX);
         ...
         case Fonctions (Choix).Nature is
            ...
            when Polaire =>
               Assign_String (TStr, Format_TETA);
         end case;
         ...
         Write (") : ");
         Readln (Ch);
         exit when UpCase (Ch) = Format_YES (1);
      end loop;
...
begin
   ...
   if MouseInit then
      ...
      ShowMouse;
      SetColor (TColor);
      OutTextXY (GetMaxX / 2, GetMaxY / 2 + 20, Format_MINS);
      OutTextXY (GetMaxX / 2, GetMaxY / 2 + 40, Format_LCCC);
      OutTextXY (GetMaxX / 2, GetMaxY / 2 + 60, Format_RCVA);
      loop
