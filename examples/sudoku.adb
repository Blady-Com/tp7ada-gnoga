...
with Sudoku_Mesg.sudoku_Strings; use Sudoku_Mesg.sudoku_Strings;
...
procedure Sudoku is
   ...
   Menu : constant array (0 .. 4) of String (1 .. 6) :=
     (Format_SLVE, Format_CNCL, Format_SAVE, Format_LOAD, Format_QUIT);
   ...
   begin
      ...
      for Ind in 0 .. Nbr - 1 loop
         ...
         SetColor (TColor);
         OutTextXY (15 + Ind * 64, 3, MotClef (Ind));
      end loop;
   ...
   procedure Clear is
      MenuC   : constant array (0 .. 1) of String (1 .. 6) := (Format_YES, Format_NO);
      Ch      : Character;
      Quitter : Boolean;
   begin
      ...
      loop
         if KeyPressed then
            Ch := UpCase (ReadKey);
            if Ch = Format_YES (1) then
               AffText (Format_DONE);
               CloseGraph;
               Halt (1);
            elsif Ch = Format_NO (1) then
               Quitter := True;
            else
               null;
            end if;
   ...
   begin
      TH := TextHeight (+Ch);
      TW := TextWidth (+Ch);
      Bar (100 + Colonne * TW, 100 + Ligne * TH, 100 + (Colonne + 1) * TW, 100 + (Ligne + 1) * TH);
   ...
   package body Grille is
      ...
      begin
         for Ligne in ((DansCarré - 1) / 3) * 3 + 1 .. ((DansCarré - 1) / 3) * 3 + 3 loop
            for Colonne in ((DansCarré - 1) rem 3) * 3 + 1 .. ((DansCarré - 1) rem 3) * 3 + 3 loop
               if Grille.RetourneValeur (Ligne, Colonne) = Valeur
                 or else
                 (Grille.RetourneValeur (Ligne, Colonne) = 0
      ...
      begin
         ...
         for Colonne in Grille.Indice loop
            if Grille.RetourneValeursPossibles (Ligne, Colonne) (Valeur) then
               Writeln
                 (Format_RULE (+1, +Ligne, +Colonne, +Grille.NumCarré (Ligne, Colonne), +Valeur));
      ...
      begin
         ...
         for Ligne in Grille.Indice loop
            if Grille.RetourneValeursPossibles (Ligne, Colonne) (Valeur) then
               Writeln
                 (Format_RULE (+2, +Ligne, +Colonne, +Grille.NumCarré (Ligne, Colonne), +Valeur));
               Grille.SupprimeValeur (Ligne, Colonne, Valeur);
            end if;
         end loop;
      ...
      begin
         Writeln (Format_SETV (+Ligne, +Colonne, +Valeur));
         ...
         for Colonne in Grille.Indice loop
            ...
            if Grille.RetourneValeur (Ligne, Colonne) /= 0 then
               for Colonne2 in Grille.Indice loop
                  if Grille.RetourneValeursPossibles (Ligne, Colonne2)
                     ...
                     if Grille.EstAligné (Carré, Valeur, Ligne, 0) then
                        for Colonne in Grille.Indice loop
                           if Grille.NumCarré (Ligne, Colonne) /= Carré then
                              if Grille.RetourneValeursPossibles (Ligne, Colonne) (Valeur) then
                                 Grille.SupprimeValeur (Ligne, Colonne, Valeur);
                                 Writeln (Format_RULE (+7, +Ligne, +Colonne, +Carré, +Valeur));
                              end if;
                           end if;
                        end loop;
                     end if;
         ...
         for Ligne in Grille.Indice loop
            ...
            if Grille.RetourneValeur (Ligne, Colonne) /= 0 then
               for Ligne2 in Grille.Indice loop
                  if Grille.RetourneValeursPossibles (Ligne2, Colonne)
                      (Grille.RetourneValeur (Ligne, Colonne))
                  then
                     Writeln
                       (Format_RULE
                     ...
                     if Grille.EstAligné (Carré, Valeur, 0, Colonne) then
                        for Ligne in Grille.Indice loop
                           if Grille.NumCarré (Ligne, Colonne) /= Carré then
                              if Grille.RetourneValeursPossibles (Ligne, Colonne) (Valeur) then
                                 Grille.SupprimeValeur (Ligne, Colonne, Valeur);
                                 Writeln (Format_RULE (+8, +Ligne, +Colonne, +Carré, +Valeur));
                              end if;
                           end if;
                        end loop;
                     end if;
            ...
            for Colonne in ((Carré - 1) rem 3) * 3 + 1 .. ((Carré - 1) rem 3) * 3 + 3 loop
               ...
               if Grille.RetourneValeur (Ligne, Colonne) /= 0 then
                  for Ligne2 in ((Carré - 1) / 3) * 3 + 1 .. ((Carré - 1) / 3) * 3 + 3 loop
                     for Colonne2 in ((Carré - 1) rem 3) * 3 + 1 .. ((Carré - 1) rem 3) * 3 + 3
                     loop
   ...
   procedure Démarre is
      T0 : Natural;
   begin
      MouseSetGraphBlock (7); -- wait cursor
      ...
      Writeln (Format_EXTI (+(HorlogeMS - T0)));
...
begin
   ...
   loop
      if KeyPressed then
         declare
            ch : constant Character := UpCase (ReadKey);
         begin
            if ch = Format_SLVE (1) then
               Démarre;
            elsif ch = Format_CNCL (1) then
               Efface;
            elsif ch = Format_SAVE (1) then
               Sauve;
            elsif ch = Format_LOAD (1) then
               Charge;
            elsif ch = Format_QUIT (1) then
               Clear;
            else
               null;
            end if;
