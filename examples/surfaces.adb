...
procedure Surfaces is
   ...
   begin
      ...
      Write (Format_NBCU (+NbrCourbes));
      Readln (NbrCourbes);
      Write (Format_ZFAC (+kz));
      Readln (kz);
      m := maxZ;
      Write (Format_FSTA (+a1));
      Readln (a1);
      Write (Format_SNDA (+a2));
      Readln (a2);
      ang1 := Pi * a1 / 180.0;
      ang2 := Pi * a2 / 180.0;
      Write (Format_XMIN (+infx));
      Readln (infx);
      Write (Format_XMAX (+supx));
      Readln (supx);
      Write (Format_YMIN (+infy));
      Readln (infy);
      Write (Format_YMAX (+supy));
      Readln (supy);
      Proj (1, 1) := Cos (ang1);
      Proj (1, 2) := -Sin (ang1);
      Proj (1, 3) := 0.0;
      ...
      Proj (2, 3) := Cos (ang2);
      minx := 1.1 * abs (Proj (1, 1)) * infx + abs (Proj (1, 2)) * infy - m * abs (Proj (1, 3));
      maxx := 1.1 * abs (Proj (1, 1)) * supx + abs (Proj (1, 2)) * supy + m * abs (Proj (1, 3));
      miny := 1.1 * abs (Proj (2, 1)) * infx + abs (Proj (2, 2)) * infy - m * abs (Proj (2, 3));
      maxy := 1.1 * abs (Proj (2, 1)) * supx + abs (Proj (2, 2)) * supy + m * abs (Proj (2, 3));
   end Calcule;
